import schedule
import time
from get_realtime_data import fetch_real_time_data
from predict_price import predict_and_act
...
def job():
    ...
    symbol = 'ETHUSDT'
    type = 'futures'
    data = fetch_real_time_data(symbol, type)
    result = predict_and_act(data)

    if result == "sell":
        order = place_order(symbol, 'SELL')
        # 이부분 텔레그램으로 메시지 보내기
        print(order)
    elif result == "buy":
        order = place_order(symbol, 'BUY')
        # 이부분 텔레그램으로 메시지 보내기
        print(order)
    else:
        print("매수 보류: 큰 가격 변동 없음")

schedule.every(1).minutes.do(job)  # 매 10분마다 'job' 함수를 실행
...
# # 결과 저장
# print(data.head())
# data.to_csv(f"{symbol}_with_indicators_2024_min.csv")
